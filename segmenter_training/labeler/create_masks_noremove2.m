...
for file_num = 1:length(files)
    ...
    file_name_fg = replace(replace(file_name, '_QPI.tiff','_predicted_fg.tif'), 'nuc_for_labeling\sample3', 'predicted2');

    file_name_dt = replace(replace(file_name, '_QPI.tiff','_predicted_dt.tif'), 'nuc_for_labeling\sample3', 'predicted2');
...
%     file_name_save = [replace(replace(file_name, '_QPI.tiff',''), 'nuc_for_labeling', 'cell_prediction_1') '_mask.tiff'];
    file_name_save = [replace(replace(file_name, '_QPI.tiff',''), 'nuc_for_labeling', 'cell_prediction_1') '_mask_noremove2.tiff'];
