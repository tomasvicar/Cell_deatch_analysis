...
def split_train_test(seed=666,config=Config(),reduction=1,reduction_self=1):
    ...
    try:
        os.makedirs(path_train+os.sep+'cellDeath')
    except:
        pass
    
    try:
        os.makedirs(path_valid+os.sep+'cellDeath')
    except:
        pass
    
    try:
        os.makedirs(path_test+os.sep+'cellDeath')
    except:
        pass
    ...
    for k in train_ind:
        ...
        name_out2=name_in.replace(config.data_orig_path,config.data_pretrain_train_valid_path + os.sep + 'train').replace(os.sep + 'PC3','').replace(os.sep + 'PNT1A','').replace(os.sep + 'cellDeath','')
    ...
    train_ind = train_ind[:int(reduction_self*len(train_ind))]
