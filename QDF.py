...
channels = {
            ...
            'Clipped:TRITC':{'dtype':np.uint16},
            'TRITC':{'dtype':np.uint16},
            'Clipped:FITC':{'dtype':np.uint16},
            'FITC':{'dtype':np.uint16},
            'Clipped:DAPI':{'dtype':np.uint16},
            'DAPI':{'dtype':np.uint16},
...
class reader():
    ...
    def build_index(self):
        ...
        """
        with open(self.filename.replace('.qdf','') + '.json', "r") as f:
            index = json.load(f)
        ...
        # self.file.seek(0)
        # for line_ind, line in enumerate(self.file):
        #     if (line_ind % 100000) == 0:
        #         print(str(int(line_ind/100000)))
        #         print(str(counter))
        #         # print(index)
